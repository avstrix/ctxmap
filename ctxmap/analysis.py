...
def render_report(store: GraphStore, out_dir: Path) -> str:
    ...
    stats = data["stats"]
    lines = [
        "# ctxmap — Knowledge Graph Report",
        "",
        f"**Nodes:** {stats['nodes']}  **Edges:** {stats['edges']}  **Files:** {stats['files']}",
        f"**Semantic layer:** {'not built — run `/ctxmap semantic`' if stats['structural_only'] else 'built'}",
        "",
    ...
    lines += [
        "",
        "---",
        "*Generated by ctxmap. Run `/ctxmap query <question>` to traverse the raw graph.*",
    ]
